...
from ..datasets import HotpotQASample, load_hotpotqa_split
from ..estimators import UPropEstimator
from ..experiments import ExperimentTracker
from ..executors import HotpotQALocalBranchingExecutor
...
from ..results_summary import summarize_records
...
def _resolved_step_output_format(config: HotpotQARunnerConfig) -> str:
...
def _resolved_step_retry_attempts(config: HotpotQARunnerConfig) -> int:
...
def _normalize_hotpotqa_answer(answer: Any) -> str | None:
    if answer is None:
        return None
...
def _hotpotqa_tdp_outcome_bucket(tdp: Any) -> str:
    normalized = _normalize_hotpotqa_answer(getattr(tdp, "final_answer", None))
...
def _majority_final_answer(estimates: Sequence[str | None]) -> str | None:
    normalized = [_normalize_hotpotqa_answer(answer) for answer in estimates]
...
def _serialize_uprop_result(sample: HotpotQASample, estimate: Any) -> dict[str, Any]:
    predicted_answer = _majority_final_answer([tdp.final_answer for tdp in estimate.tdps])
    reference_answer = _normalize_hotpotqa_answer(sample.answer)
    return {
...
def _serialize_baseline_result(sample: HotpotQASample, estimate: Any) -> dict[str, Any]:
    predicted_answer = _majority_final_answer([tdp.final_answer for tdp in estimate.tdps])
    reference_answer = _normalize_hotpotqa_answer(sample.answer)
    return {
        "sample_id": sample.sample_id,
...
async def run_hotpotqa_samples(
    samples: Sequence[HotpotQASample],
    *,
    model_client: Any,
    config: HotpotQARunnerConfig,
    tracker: ExperimentTracker | None = None,
    sample_offset: int = 0,
    sample_total: int | None = None,
    record_callback: Callable[[HotpotQASample, dict[str, Any], int, int], None] | None = None,
) -> list[dict[str, Any]]:
    shared_sampling_storage = SharedSamplingStorage(config.shared_sampling_dir)
    executor = HotpotQALocalBranchingExecutor(
        model_client,
        ...
        model_signature=_hotpotqa_model_signature(config),
    ...
    estimator = UPropEstimator(
        ...
        outcome_bucket_fn=_hotpotqa_tdp_outcome_bucket,
    )
    ...
    for index, sample in enumerate(samples, start=sample_offset + 1):
        ...
        try:
            ...
            LOGGER.warning(
                "HotpotQA sample_id=%s failed to generate due to model error code=%s retryable=%s",
                sample.sample_id,
                exc.error_code,
                exc.retryable,
            )
...
def summarize_hotpotqa_results(records: Sequence[dict[str, Any]]) -> dict[str, Any]:
    return summarize_records(records)


def write_hotpotqa_results(output_path: str, records: Sequence[dict[str, Any]]) -> Path:
...
def _build_experiment_name(config: HotpotQARunnerConfig) -> str:
    if config.experiment_name:
        return config.experiment_name
    return f"hotpotqa-{config.method}-{config.model.replace('/', '-')}-{config.split}"


def _hotpotqa_resume_fingerprint(config: HotpotQARunnerConfig) -> str:
...
def _default_run_name(config: HotpotQARunnerConfig) -> str:
    limit_label = "all" if config.limit is None else str(config.limit)
    return f"offset-{config.offset}-limit-{limit_label}-{_hotpotqa_resume_fingerprint(config)}"


def _load_sample_ids_file(path: str | Path) -> list[str]:
    sample_ids: list[str] = []
...
def _hotpotqa_model_signature(config: HotpotQARunnerConfig) -> dict[str, Any]:
...
def _resolve_output_path(config: HotpotQARunnerConfig, tracker: ExperimentTracker | None) -> str:
    if config.output_path is not None:
        return config.output_path
    if tracker is not None:
        return str(tracker.artifacts_dir / "results.jsonl")
    return f"outputs/hotpotqa-{config.method}-{config.model.replace('/', '-')}-{config.split}.jsonl"
...
def _load_hotpotqa_results(path: str | Path) -> list[dict[str, Any]]:
...
def _write_checkpoint(
    ...
    *,
    config: HotpotQARunnerConfig,
    selected_samples: Sequence[HotpotQASample],
    output_path: str,
    records: Sequence[dict[str, Any]],
) -> None:
    checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "config_fingerprint": _hotpotqa_resume_fingerprint(config),
...
def _load_checkpoint_records(
    ...
    *,
    ...
    expected_fingerprint = _hotpotqa_resume_fingerprint(config)
    if payload.get("config_fingerprint") != expected_fingerprint:
        raise ValueError(
            "Existing checkpoint belongs to a different HotpotQA configuration. Use --restart or change the run name/output path."
        )
    ...
    if payload.get("selected_sample_ids") != expected_sample_ids:
        raise ValueError(
            "Existing checkpoint does not match the selected HotpotQA samples. Use --restart or change the run name/output path."
        )
    records = payload.get("records", [])
    if not isinstance(records, list):
        raise ValueError("HotpotQA checkpoint is invalid: records must be a list.")
    return records


def _build_tracker(config: HotpotQARunnerConfig) -> ExperimentTracker | None:
...
async def run_hotpotqa(config: HotpotQARunnerConfig) -> dict[str, Any]:
    ...
    LOGGER.info(
        "Starting HotpotQA run: method=%s model=%s provider=%s split=%s limit=%s offset=%s",
        config.method,
        config.model,
        config.provider,
        config.split,
    ...
    if config.sample_ids_file:
        requested_ids = _load_sample_ids_file(config.sample_ids_file)
        samples_by_id = {sample.sample_id: sample for sample in samples}
        missing_ids = [sample_id for sample_id in requested_ids if sample_id not in samples_by_id]
        if missing_ids:
            raise ValueError(
                f"HotpotQA sample ids file contains {len(missing_ids)} id(s) not present in the selected split; "
    ...
    if not selected_samples:
        raise ValueError("No HotpotQA samples were selected for the requested split/offset/limit.")
    LOGGER.info("Selected %s HotpotQA samples from %s/%s", len(selected_samples), config.dataset_name, config.config_name)
    if tracker is not None:
        tracker.log_event(
            "samples_selected",
    ...
    if checkpoint_records:
        ...
        if dropped_checkpoint_sample_ids:
            LOGGER.info(
                "Dropping %s stale HotpotQA checkpoint record(s) without logprob state: %s",
    ...
    if checkpoint_records:
        LOGGER.info("Loaded %s checkpointed HotpotQA records from %s", len(checkpoint_records), checkpoint_path)
        if tracker is not None:
    ...
    if (
        ...
        existing_records = _load_hotpotqa_results(output_path)
        ...
        if existing_records and existing_ids == selected_ids:
            ...
            if not dropped_existing_sample_ids:
                progress_display.start(
                    ...
                    description=f"HotpotQA {config.method}",
                ...
                summary = summarize_hotpotqa_results(reusable_existing_records)
                tracker.finalize(
                    status="completed",
                    summary=summary,
                    artifacts={
            ...
            LOGGER.info(
                "Dropping %s stale HotpotQA result record(s) without logprob state from %s",
    ...
    LOGGER.info("HotpotQA resume state: %s completed, %s pending", len(all_records), len(pending_samples))
    if tracker is not None:
        tracker.log_event(
            "resume_state",
    ...
    def persist_record(sample: HotpotQASample, record: dict[str, Any], index: int, total: int) -> None:
        del total
    ...
    try:
        ...
        if pending_samples:
            ...
            new_records = await run_hotpotqa_samples(
                pending_samples,
        ...
        summary = summarize_hotpotqa_results(all_records)
        written_path = write_hotpotqa_results(output_path, all_records)
        _delete_checkpoint(checkpoint_path)
        LOGGER.info("Wrote %s HotpotQA records to %s", len(all_records), written_path)
        if tracker is not None:
        ...
        LOGGER.exception("HotpotQA run failed")
        ...
        try:
            ...
            LOGGER.warning("Failed to close HotpotQA model client: %s", exc)

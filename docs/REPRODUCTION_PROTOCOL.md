# Detailed BayesTraj reproduction protocol
...
This repository contains two complementary code layers; `src/bayestraj` is
not merely plotting code.

| Location | Role |
|---|---|
| `src/bayestraj/core.py` | Compact reference implementation of the entropy grid, Jeffreys count prior, linear-Gaussian likelihood, grid posterior fusion, LCB95 score, variance-threshold calibration, and adaptive stopping rule |
| `src/bayestraj/paper.py` and `src/bayestraj/figures.py` | Frozen-table aggregation, manuscript claims, and Figures 1–9 |
| `raw_pipeline/src/ltuq/` | Full trajectory representation, model clients, AgentBench and QA executors, dataset adapters, BayesTraj checkpointing dependencies, and all 17 submitted baseline implementations |
| `raw_pipeline/scripts/` | Raw-pool generation, cross-fitted BayesTraj evaluation, ablations, sensitivity analysis, baseline evaluation, aggregation, and report construction |
| `src/bayestraj/pipeline.py` | Orchestration layer invoked by `scripts/reproduce.py raw` |

Reviewers who only want to inspect the estimator should start with
`src/bayestraj/core.py`. Reviewers reproducing the experiment from generated
trajectories should use `scripts/reproduce.py raw`; its `--dry-run`
option prints every underlying command.
...
task–backbone–seed evaluations therefore mean `300 tasks × 3 backbones × 3
seeds`, not 300 tasks selected from a pool of 2,700. Exact revisions,
selected-row hashes, and expected registry sizes are frozen in
...
| Baseline(s) | Submitted setting (default setting from the original paper) |
|---|---|
...
`requirements.txt` is the complete dependency list for both tracks.
`requirements-fast.txt` is an optional CPU-only subset for reviewers running
only the commands above; `requirements-raw.txt` is retained as a compatible
alias to the complete list. vLLM runs in the pinned container, while
AgentBench and its services are external benchmark dependencies rather than
Python packages from this repository.
...
src/bayestraj/pipeline.py  full-pipeline orchestration
...
raw_pipeline/src/ltuq/     full experiment runtime and baseline implementations
raw_pipeline/scripts/      raw generation, evaluation, and aggregation programs
...
The complete pipeline is exposed through one driver. The driver preserves the
individual programs and logs for inspection but removes the need to copy more
than ten shell recipes from the top-level README.
...
Start the eight model endpoints and AgentBench services as described below.
Then issue the same generation command once for each currently served
backbone:

```bash
python scripts/reproduce.py raw generate \
  --backbone qwen35 --model qwen3.5:9b --ports 8000,8001,8002,8003,8004,8005,8006,8007

# After switching the endpoints to Gemma:
python scripts/reproduce.py raw generate \
  --backbone gemma3 --model gemma3:12b --ports 8000,8001,8002,8003,8004,8005,8006,8007 \
  --max-tokens 1024

# After switching the endpoints to GPT-OSS:
python scripts/reproduce.py raw generate \
  --backbone gptoss20b --model gpt-oss:20b --ports 8000,8001,8002,8003,8004,8005,8006,8007
```

Each call creates both the submitted `Z=16` pools and the separate audited
`Z=12,N=4` UProp/Degree pools for all four datasets and three seeds. It is
shard-resumable. Once all three backbones finish, one command audits the pools
and runs every BayesTraj, baseline, ablation, sensitivity, aggregation, and
report stage:
...
Use `--dry-run` with `generate` or `analyze` to inspect the exact underlying
commands. The following expanded protocol is retained as an optional audit and
...
Pass the related WebShop campaign explicitly; generated files remain outside
...
python raw_pipeline/scripts/generate_selected_methods_four_dataset_report.py \
  ...
  --webshop-root "$WEBSHOP_RUN/cells" \
